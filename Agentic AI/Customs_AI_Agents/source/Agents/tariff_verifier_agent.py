...
def tariff_verifier_agent(
    ...
    prompt = f"""
            ...
            ### DESCRIPTION COMPARISON RULES (VERY IMPORTANT)
            - Treat descriptions as **MATCHING** if they refer to the same base product.
            - Generic category terms like:
            - "Other - X"
            - "Other varieties of X"
            - "Miscellaneous X"
            **do NOT cause a mismatch** if the user description correctly states the base item (e.g., "Milk").
            - A description mismatch should be reported **ONLY IF** the product meaning is different.
            - Examples:
            - "Milk" vs "Other - Milk" → MATCH 
            - "Milk" vs "Cheese" → MISMATCH 
            - "Milk Powder" vs "Liquid Milk" → MISMATCH 

            ### STATUS RULES
            - Description mismatch → DESCRIPTION MISMATCH
            - Duty mismatch → DUTY PERCENTAGE MISMATCH
            - HS code mismatch → INCORRECT HS CODE
            - If all relevant fields match → ACCEPTED

            ### IMPORTANT
            - First evaluate ONLY the field mentioned in "Reason"
            - Stop evaluation immediately if the priority field mismatches
            - Explanation must be under 70 words
            - Compare numeric values by value, not type (1 == 1.0)

            ### RESPONSE FORMAT
            Status: <ACCEPTED / INCORRECT HS CODE / DESCRIPTION MISMATCH / DUTY PERCENTAGE MISMATCH>
            Failed_Field: <hs_code / description / duty_percentage / none>

            Correct_Value:
            - REQUIRED if Status ≠ ACCEPTED
            - MUST be copied from OFFICIAL TARIFF DATABASE DATA
            - MUST NOT be null
    ...
    try:
        ...
        # AUTO-CORRECTION (Safe, deterministic)
        if failed_field and correct_value and declaration:
            if failed_field == "description":
                declaration.goods_description = correct_value

            elif failed_field == "duty_percentage":
                declaration.hs_code_duty_fee = correct_value

            elif failed_field == "hs_code":
                declaration.hs_code = correct_value
